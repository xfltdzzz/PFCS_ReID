...
def extract_cnn_feature(model, inputs):
    ...
    outputs, outputs_up, outputs_down, outputs_mix = model(inputs)
    outputs = outputs.data.cpu()
    outputs_up = outputs_up.data.cpu()
    outputs_down = outputs_down.data.cpu()
    outputs_mix = outputs_mix.data.cpu()
    return outputs, outputs_up, outputs_down,outputs_mix
...
def extract_feats(model, data_loader, print_freq=50):
    ...
    with torch.no_grad():
        for i, (imgs, fnames, pids, _, _) in enumerate(data_loader):
            ...
            outputs,_,_,_ = extract_cnn_feature(model, imgs)
...
def extract_features(model, data_loader, print_freq=50):
    ...
    with torch.no_grad():
        for i, (imgs, fnames, pids, _, _) in enumerate(data_loader):
            ...
            outputs, outputs_up, outputs_down,output_mix = extract_cnn_feature(model, imgs)
            for fname, output, output_up, output_down,output_mix, pid in zip(fnames, outputs, outputs_up, outputs_down,output_mix, pids):
                features[fname] = output
                features_up[fname] = output_up
                features_down[fname] = output_down
                features_mix[fname] = output_mix
                labels[fname] = pid
    ...
    return features, features_up, features_down, features_mix,labels
...
class Evaluator(object):
    ...
    def evaluate(self, data_loader, query, gallery, cmc_flag=False, rerank=False):
        features, features_up, features_down,features_D, _ = extract_features(self.model, data_loader)
